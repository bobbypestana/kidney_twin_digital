...
\begin{itemize}
     \item \textit{How it works:} It starts with zero features. It tests every feature and picks the one that single-handedly explains the most variance. Then it looks for a second feature that adds the most value \textit{on top of the first}, and so on. We set a limit of maximum 12 features, to further avoid over-fitting. In this specific run, the model converged at \textbf{6 features}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Mean Absolute Error (MAE):} $10.82 \, \text{ml/min/1.73m}^2$. On average, our prediction is off by about 11 units.
    \item \textbf{$R^2$ (Signal Strength):} $0.191$. The model successfully explains ~19\% of the variation in kidney function purely from CT image data (vascular-only).
...
\hline
1 & \texttt{arterial\_venacava\_below\_kidney} & 1.96 & Point HU measurement in infra-renal IVC. \\
2 & \texttt{arterial\_right\_kidney\_vein} & 4.98 & Point HU in right renal vein. \\
3 & \texttt{arterial\_venecava\_between\_kidney\_hepatic} & 3.14 & Point HU in mid-renal Inferior Vena Cava (IVC). \\
4 & \texttt{arterial\_left\_kidney\_artery} & -1.34 & Point HU measurement in left renal artery. \\
5 & \texttt{venous\_right\_kidney\_artery} & 2.04 & Point HU (Venous phase) in right renal artery. \\
6 & \texttt{arterial\_right\_kidney\_artery} & -2.55 & Point HU measurement in right renal artery. \\
\hline
\end{tabular}
